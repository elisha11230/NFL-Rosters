...
| `top100_2025.py` | The Top 100 list plus name aliases for roster matching |
...
- **Injuries are wired but dormant.** nflverse publishes injury reports per
  season and the 2026 file does not exist until games are played.
  `build_injuries.py` looks for the current season, reports nothing when it is
  absent, and never falls back to last season -- showing 2025 injuries as current
  would be the same failure as the stale contracts CSV.
